...
type ScoringTab = "groups" | "knockouts";

export function ScoringHelpModal({ onClose }: { onClose: () => void }) {
  const { t, n, dir } = useI18n();
  const [tab, setTab] = useState<ScoringTab>("groups");
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/50 p-0 pb-[env(safe-area-inset-bottom)] sm:items-center sm:p-4 sm:pb-4">
      <div className="flex max-h-[85dvh] w-full max-w-md flex-col rounded-t-3xl bg-surface shadow-xl sm:rounded-3xl" dir={dir}>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <div className="mb-4 flex gap-1 rounded-2xl border border-border bg-bg p-1 text-sm font-medium">
            <button
              type="button"
              onClick={() => setTab("groups")}
              className={`flex-1 rounded-xl px-3 py-2 transition ${
                tab === "groups" ? "bg-surface text-ink shadow-sm" : "text-ink-soft hover:text-ink"
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 rounded-xl px-3 py-2 transition ${
                tab === "knockouts" ? "bg-surface text-ink shadow-sm" : "text-ink-soft hover:text-ink"
              }`}
            >
          ...
          {tab === "groups" ? (
            ...
            <>
              ...
              <p className="mt-3 px-1 text-[11px] text-ink-soft">
