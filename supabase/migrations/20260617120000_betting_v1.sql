-- Betting layer v1: house bets on 1X2 odds, parallel "$" balance, fully decoupled
-- from the prediction game. Prediction views (public.prediction_points, public.
-- leaderboard) are not touched.
...
-- 1) bets: one open bet per (player, match)
...
-- 2) ledger: signed amounts, balance is derived
...
-- 4) Lock trigger — bets immutable after kickoff (mirror of enforce_prediction_lock).
...
BEGIN
  IF current_setting('app.bypass_lock', true) = 'on' THEN
    ...
    RETURN NEW;
  END IF;
  -- Allow settlement updates (status flips, payout/settled_at) any time.
...
-- 5) Starting balance grant on new player.
...
-- Backfill: $100 to every existing player who doesn't already have a grant.
INSERT INTO public.bet_transactions (player_id, kind, amount, note)
...
-- 6) Settlement trigger — separate from matches_emit_finished, listens to the
--    same FINISHED transition.
...
BEGIN
  ...
  -- Settle winners and credit their payouts in one CTE so the ledger row
  -- can't be missed even on partial failure.
  WITH won AS (
  ...
  -- Losers — stake was already debited at placement.
  UPDATE public.bets SET
    status = 'lost',
    payout = 0,
    settled_at = now()
...
-- 7) RLS + grants. Bets are written by the service role via /api/public/place-bet,
--    so the ledger has no write policy at all — clients can't touch the balance
--    source directly.
