-- Global VAR Report availability, controlled from god mode. Two independent
-- switches, applied to EVERYONE:
--   var_report_visible : show the VAR Report card in the Picks feed
--   var_report_popup   : auto-show the one-time "your report is ready" popup
-- Singleton row (id = true), read by all clients; writes go through the
-- SECURITY DEFINER admin_* RPC (the client is PIN-gated only, matching the
-- other admin_* functions).
...
-- Propagate toggles live to every open client.
